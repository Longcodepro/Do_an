(() => {
  ...
  function getData() {
    ...
    let keywordFromHome = localStorage.getItem("searchKeyword"); // Sửa keywordFromHome
  ...
  function locTheoLoai(maLoai) {
    sanPhamLoc = mangsanpham.filter((sp) => sp.maMatHang === maLoai);
    trangHienTai = 1;
  ...
  function hienThiDanhMuc() {
    ...
    // Lấy danh sách mã mặt hàng (chuỗi)
    const loaiKhacNhau = [...new Set(data.map((sp) => sp.maMatHang))];
    danhmuc.innerHTML = "";

    // Hàm trợ giúp để tìm tên mặt hàng từ mã số (cần matHang là Global)
    const tenMatHang = (ma) => {
      // matHang.maMatHang là số, ma là chuỗi, nên cần so sánh cẩn thận (hoặc toString() hoặc Number())
      const item = matHang.find((mh) => mh.maMatHang.toString() === ma);
      return item ? item.tenMatHang : "Chưa đặt tên";
    };

    loaiKhacNhau.forEach((maMH) => {
      const spDaiDien = data.find((sp) => sp.maMatHang === maMH);
      const tenDM = tenMatHang(maMH); // Lấy tên danh mục chính xác
