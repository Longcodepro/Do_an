function quanLyGiaBan() {
  ...
  <select id="locLoai" class="chon-loai">
    ...
    ${dsLoai
      .map((l) => `<option value="${l.MA_MAT_HANG}">${l.TEN_MAT_HANG}</option>`)
      .join("")}
  </select>
  ...
  function hienThi(ds) {
    ...
    ds.forEach(sp => {
      ...
      // Tìm sản phẩm trong bảng nhập
      const nhapSP = dsNhap.find(
        n => chuanMa(n.MA_SAN_PHAM) === chuanMa(sp.MA_SAN_PHAM)
      );
  
      // 👉 Giá vốn là GIA_NHAP
      const giaVon = nhapSP ? parseFloat(nhapSP.GIA_NHAP) : 0;
      ...
      tr.innerHTML = `
        ...
        <td>${sp.TEN_SP}</td>
        <td>${tenLoai}</td>
        <td>${giaVon ? giaVon.toLocaleString("vi-VN") : "Chưa có"}</td>
